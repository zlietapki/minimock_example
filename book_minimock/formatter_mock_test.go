...
import (
	...
	mm_logrus "github.com/sirupsen/logrus"
)

// FormatterMock implements logrus.Formatter
type FormatterMock struct {
	t minimock.Tester

	funcFormat          func(ep1 *mm_logrus.Entry) (ba1 []byte, err error)
	inspectFuncFormat   func(ep1 *mm_logrus.Entry)
	afterFormatCounter  uint64
	beforeFormatCounter uint64
	FormatMock          mFormatterMockFormat
}

// NewFormatterMock returns a mock for logrus.Formatter
...
func (mmFormat *mFormatterMockFormat) Expect(ep1 *mm_logrus.Entry) *mFormatterMockFormat {
	...
	mmFormat.defaultExpectation.params = &FormatterMockFormatParams{ep1}
...
func (mmFormat *mFormatterMockFormat) Inspect(f func(ep1 *mm_logrus.Entry)) *mFormatterMockFormat {
...
func (mmFormat *mFormatterMockFormat) Return(ba1 []byte, err error) *FormatterMock {
	if mmFormat.mock.funcFormat != nil {
		mmFormat.mock.t.Fatalf("FormatterMock.Format mock is already set by Set")
	}
	...
	mmFormat.defaultExpectation.results = &FormatterMockFormatResults{ba1, err}
	return mmFormat.mock
}

//Set uses given function f to mock the Formatter.Format method
func (mmFormat *mFormatterMockFormat) Set(f func(ep1 *mm_logrus.Entry) (ba1 []byte, err error)) *FormatterMock {
	if mmFormat.defaultExpectation != nil {
...
// Then helper
func (mmFormat *mFormatterMockFormat) When(ep1 *mm_logrus.Entry) *FormatterMockFormatExpectation {
	if mmFormat.mock.funcFormat != nil {
		mmFormat.mock.t.Fatalf("FormatterMock.Format mock is already set by Set")
	}
	...
	expectation := &FormatterMockFormatExpectation{
		mock:   mmFormat.mock,
		params: &FormatterMockFormatParams{ep1},
	}
...
func (e *FormatterMockFormatExpectation) Then(ba1 []byte, err error) *FormatterMock {
	e.results = &FormatterMockFormatResults{ba1, err}
	return e.mock
}

// Format implements logrus.Formatter
func (mmFormat *FormatterMock) Format(ep1 *mm_logrus.Entry) (ba1 []byte, err error) {
	...
	if mmFormat.inspectFuncFormat != nil {
		mmFormat.inspectFuncFormat(ep1)
	}

	mm_params := &FormatterMockFormatParams{ep1}
	...
	if mmFormat.FormatMock.defaultExpectation != nil {
		...
		mm_got := FormatterMockFormatParams{ep1}
	...
	if mmFormat.funcFormat != nil {
		return mmFormat.funcFormat(ep1)
	}
	mmFormat.t.Fatalf("Unexpected call to FormatterMock.Format. %v", ep1)
	return
}
